...
loc <- file.path(find_project_folder(), "clinical", "talks", "12-07-22_typical-avnrt")
```
...
![](valsalva-hemodynamics.png){fig-align="center" .r-stretch}
...
# The Fork in the Road {.r-fit-text}

## Surface Electrocardiograms

## {background-image="ecg-case-nsr.png" background-size="cover"}

## {background-image="ecg-case-svt-old.png" background-size="cover"}

## {background-image="ecg-case-svt-new.png" background-size="cover"}
...
```{r}
#| cache: true
x <- read_lspro(file.path(loc, "m3916-svt-initiation.txt"))
...
```{r}
#| cache: true
x <- read_lspro(file.path(loc, "m3916-avn-erp-s1-600ms-s2-220ms.txt"))
...
```{r}
decremental_800 <- 
	read_lspro(file.path(loc, "m3916-cs-pacing-800.txt")) |>
	{\(.x) melt_egm(.x$header, .x$signal, format = "lspro")}()

decremental_700 <- 
	read_lspro(file.path(loc, "m3916-cs-pace-decremental-700ms.txt")) |>
	{\(.x) melt_egm(.x$header, .x$signal, format = "lspro")}()

decremental_640 <- 
	read_lspro(file.path(loc, "m3916-cs-pace-decremental-640ms.txt")) |>
	{\(.x) melt_egm(.x$header, .x$signal, format = "lspro")}()

decremental_400 <- 
	read_lspro(file.path(loc, "m3916-cs-pace-decremental-400ms-jump.txt")) |>
	{\(.x) melt_egm(.x$header, .x$signal, format = "lspro")}()
```
